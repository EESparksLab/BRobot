...
CAM_SN = 22624783 #Serial Number of the camera
class RosReader(Node):
    def __init__(self):
      super().__init__('lefteye_driver')
      self.cam_sub = self.create_subscription(Bool, '/lefteye_cmd', camera_stream , 10)
...
def acquire_images(cam, nodemap, nodemap_tldevice):
    ...
    try:
        ...
        #  than other node types. Two nodes must be retrieved: f_Mono8irst, the
        ...
        def camera_stream(state):
            ...
            if(state_bool):
                if img_series_count == 0:
                    ...
                    os.mkdir('/spinPics/leftCam/'+ img_series_str, 0o777)
                    os.chmod('/spinPics/leftCam/'+ img_series_str, 0o777)
                try:
                    ...
                    if image_result.IsIncomplete():
                        ...
                        filename = '/spinPics/leftCam/%s/%d.jpg' % (img_series_str,img_series_count)
                        #  Save image
                        #
                        #  *** NOTES ***
